...
class NetworkMonitor {
    ...
    async testEmailSettings() {
        try {
            ...
            console.log('Testing email settings:', testSettings);
            
            // EmailService를 사용하여 실제 이메일 테스트 수행
            const emailService = new EmailService();
            const result = await emailService.testEmailConfiguration(testSettings);
            
            this.addLog('info', '이메일 설정 테스트 성공', result);
            this.showNotification(`이메일 설정 테스트가 성공했습니다! 테스트 이메일이 ${toEmails.join(', ')}로 발송되었습니다.`, 'success');
            
            console.log('Email settings test completed successfully');
    ...
    async sendAlert(host, message) {
        if (!this.emailSettings || !this.emailSettings.is_enabled) {
            this.addLog('debug', '이메일 알림 비활성화됨', { hostId: host.id, message });
            return;
        }
        ...
        if (lastAlertTime && (now - lastAlertTime) < 15 * 60 * 1000) {
            this.addLog('debug', '스팸 방지를 위해 알림 발송 건너뜀', { 
                hostId: host.id, 
                timeSinceLastAlert: now - lastAlertTime 
            });
            return;
        }
        ...
        try {
            ...
            const emailService = new EmailService();
            const result = await emailService.sendAlertEmail(this.emailSettings, host, message);
            
            this.addLog('info', '이메일 알림 발송 성공', { 
                hostName: host.name, 
                messageId: result.result?.messageId 
            });
            
            this.showNotification(`${host.name}에 대한 이메일 알림이 성공적으로 발송되었습니다.`, 'success');
            
            this.lastAlertTimes.set(host.id, now);
            
        } catch (error) {
            this.addLog('error', '이메일 알림 발송 실패', { 
                hostName: host.name, 
                error: error.message 
            });
            this.showNotification(`${host.name}에 대한 이메일 알림 발송에 실패했습니다: ${error.message}`, 'error');
        }
    }
